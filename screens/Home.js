...
import Logo from "../assets/logo_admin.jpg";
...
function App({ navigation }) {
  ...
  const { data: newCoordinate } = useSubscription(DELETE_SUBSCRIPTION, {
    onData: ({ data }) => {
      setList([
        ...list.filter((item) => item.bus_id !== data?.data?.deleteBus?.bus_id),
      ]);
      return;
    },
  });
  ...
  const onChangeText = (string) => {
    setSearch(
      filter(
        list,
        (e) =>
          includes(e.bus_name.toLowerCase(), string.toLowerCase()) ||
          includes(e.bus_id.toLowerCase(), string.toLowerCase())
      )
    );
  };
          ...
          {searchResult.length > 0 ? (
            searchResult.map((item) => (
              ...
              >
                <Text style={{ color: "#333" }}>
                  Bus #:{" "}
                  <Text style={{ fontWeight: "bold" }}>{item.bus_id}</Text>
                </Text>
                <Text style={{ color: "#333" }}>
                  Bus Name:{" "}
            ...
            <Text style={{ color: "#333" }}>No item found</Text>
          )}
        </ScrollView>
      )}
      <SwipeablePanel
        ...
        <View style={{ padding: 15 }}>
          ...
          <View style={{ marginTop: 10, flexDirection: "row" }}>
            ...
            >
              <Image
                source={{
                  uri: `http://${SERVER_ADDED}/images/bus-${selected?.id}.png?${selected?.createdAt}`,
                }}
            ...
            <View>
              ...
              <ItemData
                label="Passenger"
                value={(selected?.passenger_count || 0) + "/45"}
              />
          ...
          <View style={{ marginTop: 10, flexDirection: "row" }}>
            ...
            >
              <Image
                source={{
                  uri: `http://${SERVER_ADDED}/images/driver-${selected?.id}.png?${selected?.createdAt}`,
                }}
                style={{ height: 60, width: 60 }}
              />
            </View>
          ...
          <View style={{ marginTop: 10, flexDirection: "row" }}>
            ...
            >
              <Image
                source={{
                  uri: `http://${SERVER_ADDED}/images/conductor-${selected?.id}.png?${selected?.createdAt}`,
                }}
                style={{ height: 60, width: 60 }}
              />
            </View>
            <View>
              ...
              <ItemData
                label="Contact Number"
                value={selected?.conductor_contact}
              />
            </View>
          </View>
        </View>
